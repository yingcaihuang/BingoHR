...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `is_microsoft` tinyint NOT NULL DEFAULT '0' COMMENT '是否来自Microsoft Entra ID登录, 0否1是',
  `otp_enable` tinyint NOT NULL DEFAULT '1' COMMENT 'mfa开关，0启用，1关闭.',
  `otp_secret` varchar(256) NOT NULL DEFAULT '' COMMENT '2fa认证时的secret',
  `otp_url` varchar(1024) NOT NULL DEFAULT '' COMMENT '2fa认证时的url',
  `create_uid` int unsigned NOT NULL DEFAULT '0' COMMENT '创建人',
  `create_time` int unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
  `update_time` int unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
  PRIMARY KEY (`id`)
